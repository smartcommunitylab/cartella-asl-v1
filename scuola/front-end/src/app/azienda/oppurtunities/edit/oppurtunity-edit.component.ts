...
import { IOffer, Competenza } from '../../../shared/interfaces';
...
import { SceltaAziendaModalComponent } from './scelta-azienda-modal/scelta-azienda-modal.component';
...
export class OppurtunityEditComponent implements OnInit {
      ...
      this.dataService.getData("tipologiaAzienda").subscribe((data) => {
        ...
        if (id == 0) { //new
          ...
          this.offer = {
            ...
            referente: '',
            referenteCF: '',
            coordinate: config.defaultPosition,
            oraInizio: '',
            oraFine: ''
          };
          setTimeout(() => {
  ...
  allValidated() {
    return this.offer.start && this.offer.end &&
      ...
      this.offer.titolo && this.offer.tipologia && this.offer.ore && this.offer.postiDisponibili && this.offer.azienda && this.offer.referente;
  }
  ...
  aziendaClick() {
    this.aziendaClicked = true;
    const modalRef = this.modalService.open(SceltaAziendaModalComponent, { size: "lg" });
    modalRef.componentInstance.onAdd.subscribe((azienda) => {
      this.offer.azienda = azienda;
      if (azienda.coordinate && azienda.coordinate.latitude && azienda.coordinate.longitude) {
        this.offer.coordinate = azienda.coordinate;
      } else { //default.
        this.offer.coordinate = config.defaultPosition;
      }
